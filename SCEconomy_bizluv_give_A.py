...
class Economy:
    ...
    def __init__(self,
        ...
        if bgrid is not None: self.bgrid = bgrid
        ...
        if is_to_ib is not None: self.is_to_ib = is_to_ib
    ...
    def __set_default_parameters__(self):
        ...
        self.bgrid = np.load('./input_data/bgrid.npy')
        ...
        #pi(t,t+1)
        self.prob = np.load('./input_data/transition_matrix_bizluv.npy')
        ...
        self.is_to_iz = np.load('./input_data/is_to_iz_b.npy')
        self.is_to_ieps = np.load('./input_data/is_to_ieps_b.npy')
        self.is_to_ib = np.load('./input_data/is_to_ib_b.npy')
            ...
            for ip in range(assigned_state_range[0], assigned_state_range[1]):
                ...
                for ian in range(num_a):
                    for ikapn in range(num_kap):
                        ...
                        b = bgrid[is_to_ib[istate]]
                        an = agrid[ian]
                        kapn = kapgrid[ikapn]

                        state = [a, an, kap, kapn, z, b]
    ...
    #     ###end codes to generate shocks###
        ...
        ###load shock data###
        data_i_s_import = np.load('./input_data/data_i_s_bizluv.npy')
        ...
        def calc(data_a_, data_kap_, data_i_s_, data_is_c_):
            ...
            for t in range(1, sim_time):
                for i in range(num_pop_assigned):
                    ...
                    val_s = (get_sstatic([a, an_s, kap, kapn_s, z, b])[0]    + fem2d_peval(an_s, kapn_s, agrid, kapgrid, EV[:,:,istate])) **(1./(1.- mu))
        ...
        if rank == 0:
            ...
            for i in range(num_total_pop):
                ...
                if data_is_c[i, t]: 
                    ...
                    data_ss[i,4] = kapn
                    data_ss[i,5] = z
                    data_ss[i,6:15] = get_sstatic([a, an, kap, kapn, z, b])[1:]
                ...
                for ikap, kap in enumerate(kapgrid):
                    for istate in range(num_s):
                        ...
                        if val_s >= val_c:
                            ...
                            u, cc, cs, cagg, l, mx, my, x, ks, ys = get_sstatic([a, an, kap, kapn, z, b])
    ...
    #                         d_after_tax[ia, ikap, istate] = 0.0
                        ...
                        for istate_n in range(num_s):
                            ...
                            vsp = (get_sstatic([an, anp_s, kapn, kapnp_s, zp, bp])[0]    + fem2d_peval(anp_s, kapnp_s, agrid, kapgrid, EV[:,:,istate_n])) **(1./(1.- mu))
                            ...
                            if vsp >= vcp:
                                ...
                                u, cc, cs, cagg, l, mx, my, x, ks, ys = get_sstatic([an, anp, kapn, kapnp, zp, bp])
